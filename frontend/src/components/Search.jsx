...
export default function Search({ setResults, setRecipientLocation, setUserHasSearched, setIsSearching }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Donors collection allows public reads — no auth required
      ...
      if (err?.name === "GeolocationError" || typeof err?.code === "number") {
  ...
  const selectClass =
    "w-full py-3.5 px-4 text-sm border border-slate-200 rounded-xl bg-slate-50 text-slate-900 cursor-pointer transition-all duration-200 focus:outline-none focus:border-red-500 focus:ring-4 focus:ring-red-500/10 focus:bg-white font-semibold appearance-none shadow-sm hover:border-slate-300";
      ...
      <div className="max-w-3xl mx-auto px-6">
        <div className="text-center max-w-lg mx-auto mb-12">
          ...
          <p className="text-sm text-slate-500 leading-relaxed font-medium">
            Enter your required blood type and search radius. We will search nearby active donors in real-time.
          </p>
        </div>
